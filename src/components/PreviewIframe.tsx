...
import { Smartphone, Tablet, Laptop, Monitor, ChevronDown, QrCode } from "lucide-react";
...
const PreviewIframe: React.FC<PreviewIframeProps> = ({
  ...
  const displayUrl = port
    ...
    : isMinPrograme
      ...
      : t('preview.noserver');
  const expoUrl = port ? `exp://localhost:${port}` : "";
  ...
  return (
    <div     className="preview-container w-full h-full relative flex flex-col overflow-hidden">
      ...
      {showExpo && (
        ...
        >
          <div
            className="bg-white dark:bg-[#1a1a1c] p-4 rounded"
            onClick={(e) => e.stopPropagation()}
          >
            {expoUrl ? (
              <>
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(expoUrl)}&size=200x200`}
                  alt="Expo QR"
                  className="mx-auto"
                />
                <div className="mt-2 text-center text-sm text-gray-600 dark:text-gray-300">
                  {t("preview.expoPreview")}
                </div>
              </>
            ) : (
            ...
            <button
              className="mt-4 px-4 py-2 rounded bg-gray-100 dark:bg-[#2c2c2c] w-full"
              onClick={() => setShowExpo(false)}
            >
              {t("header.close")}
            </button>
