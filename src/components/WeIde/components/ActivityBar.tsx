import { Files, Search, Terminal, Github, FileCode } from "lucide-react";

import { Tooltip } from "./Tooltip";
import { cn } from "@/utils/cn";

interface ActivityBarProps {
  activeView: "files" | "search" | "snippets";
  showTerminal: boolean;
  onViewChange: (view: "files" | "search" | "snippets") => void;
...
export function ActivityBar({
  ...
  return (
    <div className="w-12 bg-[#f6f6f6] dark:bg-[#1a1a1c] flex flex-col items-center py-2 border-r border-[#e4e4e4] dark:border-[#18181a]">
      ...
      <Tooltip content="Snippets" side="right">
        <button
          aria-label="Snippets"
          className={cn(
            "p-1.5 rounded-md mb-2 transition-all duration-200 relative group",
