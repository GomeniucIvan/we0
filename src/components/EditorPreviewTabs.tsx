...
const EditorPreviewTabs: React.FC = () => {
  ...
  const [showIframe, setShowIframe] = useState<string>("preview");
  const [frameStyleMap, setFrameStyleMap] = useState<Record<string, string>>({
    editor: "translate-x-full opacity-100",
    weApi: "translate-x-full opacity-100",
    preview: "translate-x-0 opacity-100",
  ...
  const openWeChatEditor = async () => {
    ...
    try {
      ...
      if (
        getFiles().includes("app.json") ||
        getFiles().includes("miniprogram/app.json")
      ) {
