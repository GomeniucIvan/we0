...
export function GeneralSettings() {
  ...
  const [formData, setFormData] = useState<FormData>(() => {
    ...
    return {
      ...
      deepseekKey: savedKeys.deepseek || '',
      ...
      appBaseUrl: savedData.appBaseUrl || 'http://localhost:3000',
      language: savedData.language || 'en',
      projectSavePath: savedData.projectSavePath || ''
    };
  });
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          claude: formData.claudeKey || "",
          deepseek: formData.deepseekKey || "",
        })
      );
    } catch (error) {
      localStorage.setItem(
        ...
        <div className="p-4 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <CustomInput
              ...
              value={formData.appBaseUrl || "http://localhost:3000"}
              onChange={(e) => setFormData({ ...formData, appBaseUrl: e.target.value })}
            />
          </div>

          {/* Project Save Path 设置 */}
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
              {t("settings.projectSavePath")}
            </label>
            <CustomInput
              placeholder="/path/to/projects"
              value={formData.projectSavePath || ""}
              onChange={(e) =>
                setFormData({ ...formData, projectSavePath: e.target.value })
              }
            />
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, deepseekKey: e.target.value })}
                    className={classNames(
                      "w-full px-3 py-2 text-sm rounded-md pr-10",
                      "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700",
                      "focus:outline-none focus:ring-2 focus:ring-purple-500/50",
