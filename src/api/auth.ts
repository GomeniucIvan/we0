...
export const authService = {
  async login(email: string, password: string) {
    const res = await fetch(`${getAppBaseUrl()}/api/auth/login`, {
      method: "PUT",
  ...
  async getUserInfo(token: string): Promise<User> {
    try {
      const res = await fetch(`${getAppBaseUrl()}/api/user`, {
        method: "GET",
        headers: {
  ...
  async register(username: string, email: string, password: string) {
    const res = await fetch(`${getAppBaseUrl()}/api/auth/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async updatePassword(email: string, oldPassword: string, newPassword: string) {
    const res = await fetch(`${getAppBaseUrl()}/api/auth/update-password`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ 
        email, 
